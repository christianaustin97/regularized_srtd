"""
    ...
    Implements the regularized SRTD formulation and recommended iterative 
    ...
    The regularization was introduced in Austin (2026) in the analysis 
    of the case with inflow velocity data.
    ...
    auxiliary pressure pi, then a (regularized) linear transport 
    equation for the true pressure, and finally a linear transport 
    ...
    Dirichlet boundary conditions. More geometries where the tangential 
    velocity is not assumed are coming soon, since the regularization is
    intended to work with those with inflow velocities. 
"""
...
def oldroyd_3_JB_reg_SRTD(h, rad, ecc, s, eta, l1, mu1, max_iter, tol, epsilon):
    # h is the meshsize
    # rad is the radius of the inner circle (outer radius always =1)
    # ecc is the eccentricity of the inner circle
...
def oldroyd_3_LDC_reg_SRTD(h, s, eta, l1, mu1, max_iter, tol, epsilon):
